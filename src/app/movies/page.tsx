...
const getMovies = async () => {
  ...
  const movieCards = await Promise.all(movies.map((movie:Movie) => genreFetch(movie)));
  return movieCards
}

const genreFetch = async(movie: Movie) => {
  const genreIDs = Array.isArray(movie.genre) ? movie.genre.map(genre => genre.genreID) : [movie.genre.genreID];
  const genreNames = await Promise.all(genreIDs.map(async (genreID) => {
    ...
    <>
      <main>
        <header className="home-header">
          ...
          <Image height={40} width={50} alt="profile pic" src={user?.picture ?? `https://res.cloudinary.com/dy87deadk/image/upload/v1715339706/kxseok21pawjrxvytn6e.jpg`} className="profile-pic" />
        </header>
        <section className="section-movies-rated">
          <p>Movies you've already rated:</p>
